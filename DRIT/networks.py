...
# SOURCE: https://github.com/HsinYingLee/DRIT/blob/master/src/networks.py
...
#------------------------- Basic Functions -------------------------
####################################################################
def get_scheduler(optimizer, cfg, cur_ep=-1):
  if cfg["LR_POLICY"] == 'lambda':
    def lambda_rule(ep):
      lr_l = 1.0 - max(0, ep - cfg["EPOCHS_DECAY"]) / float(cfg["EPOCHS"] - cfg["EPOCHS_DECAY"] + 1)
      return lr_l
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule, last_epoch=cur_ep)
  elif cfg["LR_POLICY"] == 'step':
    scheduler = lr_scheduler.StepLR(optimizer, step_size=cfg["EPOCHS_DECAY"], gamma=0.1, last_epoch=cur_ep)
  else:
...
def get_non_linearity(layer_type='relu'):
  if layer_type == 'relu':
    nl_layer = functools.partial(nn.ReLU, inplace=False)  # FIX: Add inplace=False
  elif layer_type == 'lrelu':
    nl_layer = functools.partial(nn.LeakyReLU, negative_slope=0.2, inplace=False)
  elif layer_type == 'elu':
    nl_layer = functools.partial(nn.ELU, inplace=False)  # FIX: Add inplace=False
  else:
...
class LeakyReLUConv2d(nn.Module):
  def __init__(self, n_in, n_out, kernel_size, stride, padding=0, norm='None', sn=False):
    ...
    if norm == 'Instance':  # FIX: was 'norm' == 'Instance' which always evaluates to False!
      model += [nn.InstanceNorm2d(n_out, affine=False)]
    model += [nn.LeakyReLU(inplace=False)]  # FIX: Add inplace=False explicitly 
    self.model = nn.Sequential(*model)
...
# CHANEGD FROM ORIGINAL: to avoid in-place operations
class MisINSResBlock(nn.Module):
  ...
  def __init__(self, dim, dim_extra, stride=1, dropout=0.0):
    ...
    # Replace Sequential with individual components
    self.conv_blk1_1 = self.conv1x1(dim + dim_extra, dim + dim_extra)
    self.conv_blk1_2 = self.conv1x1(dim + dim_extra, dim)
    self.conv_blk2_1 = self.conv1x1(dim + dim_extra, dim + dim_extra)
    self.conv_blk2_2 = self.conv1x1(dim + dim_extra, dim)
    ...
    self.conv_blk1_1.apply(gaussian_weights_init)
    self.conv_blk1_2.apply(gaussian_weights_init)
    self.conv_blk2_1.apply(gaussian_weights_init)
    self.conv_blk2_2.apply(gaussian_weights_init)
  ...
  def forward(self, x, z):
    ...
    # blk1 - explicit implementation to avoid in-place operations
    cat1 = torch.cat([o1, z_expand], dim=1)
    temp1 = self.conv_blk1_1(cat1)
    temp1 = F.relu(temp1, inplace=False)  # Explicitly use F.relu with inplace=False
    o2 = self.conv_blk1_2(temp1)
    o2 = F.relu(o2, inplace=False)
    
    # Second block
    o3 = self.conv2(o2)
    
    # blk2 - explicit implementation to avoid in-place operations
    cat2 = torch.cat([o3, z_expand], dim=1)
    temp2 = self.conv_blk2_1(cat2)
    temp2 = F.relu(temp2, inplace=False)
    out = self.conv_blk2_2(temp2)
    out = F.relu(out, inplace=False)
    
    # Create a new tensor for the final output
    result = out + residual
    return result
