...
def train_model(
    ...
    """
        ...
        # Define helper function for margin scheduling
    def get_scheduled_margin(current_epoch):
        """Calculate margin based on current training progress - INCREASING over time"""
    ...
    for epoch in range(n_epochs):
        ...
        if margin_scheduling:
            ...
            # Log margin to wandb
            if log_to_wandb:
                wandb.log({"margin": current_margin, "epoch": epoch})
            ...
            for i, positive_label in enumerate(current_batch_labels):
                ...
                if mining_strat == "Random":
                    ...
                    if len(negative_indices) > 0:
                        ...
                        # Define margin
                        margin = wandb.config.initial_margin  # Adjust if needed

                        # Find negatives within the margin (semi-hard negatives)
                        semi_hard_mask = (dists < (positive_distance + margin))
                        semi_hard_dists = dists[semi_hard_mask]

                        if semi_hard_dists.numel() > 0:
                            # Pick the hardest among semi-hard (i.e., largest distance in this subset)
                            hard_idx_in_masked = torch.argmax(semi_hard_dists).item()
                            ...
                            print(f"No semi-hard negative found for label {positive_label}. Increasing margin..")

                            temp_margin = margin + 0.3
                            ...
                            if semi_hard_dists.numel() > 0:
                                ...
                                print(f"No semi-hard negative found for label {positive_label}. Choosing Random..")
        ...
        if (epoch + 1) % tsne_interval == 0:
            visualize_tsne(model, device, ilo_dataset, train_loader, 
                          trained=True, log_to_wandb=log_to_wandb, n_epochs=epoch+1, set_name="training")
            visualize_tsne(model, device, ilo_dataset, val_loader, 
                          trained=True, log_to_wandb=log_to_wandb, n_epochs=epoch+1, set_name="validation")
...
if __name__ == "__main__":
    ...
    try:
        ...
        wandb.init(project='MBOD-cl', name='TEST-rand-Nsched',
            config={
                "batch_size": 32,
                "n_epochs": 250,
                "learning_rate": 1e-4,
                ...
                "scheduling_fraction": 0.5,  # Complete scheduling in first x% of training
                "mining": "Random",
                "augmentations": False
            })
